    ...
    <div className="space-y-6">
      <Card title={t('profile.jello.overviewTitle')}>
        <TileGrid>
          ...
          <Tile label={t('profile.jello.levels')} text={`${levelsUnlocked} / ${missions.length}`} />
          <Tile label={t('profile.jello.skins')} text={`${skinsUnlocked} / ${skins.length}`} />
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        ...
        {mission.isInfinity ? (
          <span className="text-xs text-pink-400" title={t('profile.jello.endless')} aria-hidden>
            ∞
          </span>
        ) : mission.maxSkins > 0 ? (
