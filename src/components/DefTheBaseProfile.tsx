    ...
    <div className="space-y-6">
      <Card title={t('profile.scoreTitle')}>
        <TileGrid>
          ...
          <Tile label={t('profile.score.structuresBuilt')} value={data.score?.structuresBuilt} />
          ...
          <Tile
            ...
            text={
              data.state?.difficulty
                ? t(`profile.difficulty.${data.state.difficulty}`, data.state.difficulty)
                : undefined
            }
          />
          <Tile label={t('profile.state.metal')} value={data.state?.metal} />
          <Tile label={t('profile.state.mainHallHealth')} value={data.state?.mainHallHealth} />
          <Tile label={t('profile.state.adGold')} value={data.state?.adGold} />
          <Tile label={t('profile.missions')} text={`${missionsDone} / ${missionsTotal}`} />
        </TileGrid>
      </Card>
